(function (w, d) {
	...
	JC = {
		...
		Dom : {
			...
			setOpacity : function (obj, val) {  //隐藏（透明） obj
				var vals = (typeof val === "number" && val <= 100 && val >= 0) ? val : 100;
				if (!obj) {
					return;
				}
				if (ie) {
			...
			hasAttr : function (obj, attr) {  //obj是否包含 attr属性 返回对象
				obj.getAttribute(attr);
				return obj;
			},
			setAttr : function (obj, attr) {  //有问题！！！！！！！！！！！！
				var self = this;
					if (attr === 'class') {
						self.addClass(obj, attr);
					} else {
						obj.setAttribute(i, attr);
					}
				return obj;
			},
			removeAttr : function (id, attr,value) {  //删除属性 id  attr为属性
				var obj = this.$(id);
				obj.removeAttribute(attr);
				
				return obj;
			},
			getClass : function (c, id) {  //查找带有class的dom (c为class ，id为id)
				var all = id ? d.getElementById(id).getElementsByTagName("*") : d.getElementsByTagName("*"),
				str = "",
				n = [];
				for (var i = 0; i < all.length; i++) {
					if (JC.Dom.hasClass(all[i], c)) {
						n.push(all[i]);
					}
				}
				return n;
			},
			addClass : function (id, str) { //添加class id为id str为添加的class名称
				var obj = this.$(id);
				if (!obj) {
					return;
				}
			...
			removeClass : function (id, str) {  //删除class， id为id，str为class名
				var obj = this.$(id);
				if (!obj) {
					return;
				}
			...
			hasClass : function (id, str) {  //查看id的dom中是否包含str的class名
				if (!id) {
					return;
				}
				var obj = this.$(id);
			...
			html : function (id, html) {  //html 替换  id为id，下的html替换
				var obj = this.$(id);
				if (html) {
					obj.innerHTML = html;
				} else {
		...
		getScript : function (obj, callback, order) {
			...
			function add() {
				...
				var script = JC.Dom.createElement("script", {
						'src' : (str ? obj : arr[num]),
